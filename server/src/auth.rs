...
mod tests {
    ...
    fn valid_token_round_trips() {
        ...
        let t = mint(secret, &claims("enma", 10_000));
        assert_eq!(verify(secret, "enma", 9_999, &t).unwrap().workspace, "ws1");
    }
    ...
    fn rejects_wrong_secret_expiry_and_audience() {
        let secret = b"platform-secret";
        let t = mint(secret, &claims("enma", 10_000));
        assert!(verify(b"other", "enma", 1, &t).is_none(), "wrong secret");
        assert!(verify(secret, "enma", 10_001, &t).is_none(), "expired");
        assert!(verify(secret, "satori", 1, &t).is_none(), "wrong audience");
        assert!(verify(secret, "enma", 1, "garbage").is_none(), "malformed");
    }
}
